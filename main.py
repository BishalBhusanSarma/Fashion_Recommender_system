...
# Function to load an image from a URL and process it
...
if image_url or uploaded_file:
    ...
    if img:
        ...
        for i, file_idx in enumerate(indices[0]):
            ...
            # Read and process the recommended image
            temp_img = cv2.imread(file_path)
            if temp_img is None:
                st.error(f"Could not read image from path: {file_path}")
                continue
